...
Каноническая и проверочная установка используют один `skillset_file_contract.py`: служебные `tests/`, `evals` и пять версионируемых точных путей, `.env`, файлы учётных данных и закрытых ключей, `*.pyc`, `__pycache__` и другие временные файлы или секреты не копируются и не участвуют в пользовательском составе; `.env.example` остаётся разрешённым публичным примером. Точный список служебных файлов учитывает имя скилла и полный относительный путь, поэтому обычные JSON, схемы и скрипты не исключаются по совпадению имени или расширения. Исходный профиль валидатора проверяет все служебные материалы перед публикацией, а явно выбранный рабочий профиль проверяет их отсутствие и не считается полной проверкой исходников и выпуска. Перед первой записью запрещаются `/`, домашний каталог, символическая ссылка в цели или скилле и файл вместо каталога.
