...
7. **Distribution exclusion не отключает source guard.** Repository-wide public-source validation перечисляет development paths явно, поэтому запрещённый исходный документ нельзя спрятать в `tests/`, хотя этот каталог не устанавливается пользователю.
...
- **Development exclusion случайно ослабит public-source gate.** Repository validation использует `include_development=True`, а отдельный regression проверяет запрещённый материал внутри `tests/`.
