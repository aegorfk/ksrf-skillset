...
`README.md`, `docs/KSRF_SKILLS_METHODOLOGY.md`, and `docs/KSRF_PROJECT_WORK_AND_PUBLIC_SOURCES.md` MUST state that the seven installed cards are retrospective and were prepared after the later acts became known, so they are not evidence of blind evaluation or predictive accuracy. The same files MUST state that development evals are source-only and excluded from user installation. If a registered blind workflow is named, the documentation MUST explain in plain Russian that it requires a separate frozen input without the known outcome and cannot be inferred from the seven cards.

#### Scenario: User reads public methodology documentation

- **WHEN** a reader compares the seven retrospective cards with the described expert evaluation workflow
...
The current `ksrf-explore-arguments/evals/evals.json`, `ksrf-explore-arguments/evals/trigger-evals.json`, and `ksrf-complaint-qa/evals/evals.json` MUST remain digest-bound after review, validated by the source profile, and excluded from the runtime payload. The seven example filenames and owning-skill backlinks MUST remain stable, and clean-room installation MUST contain the examples but no `evals` directory.
...
- **THEN** all seven retrospective cards are present, the private complaints and explanations remain absent, and no `evals` directory is installed
