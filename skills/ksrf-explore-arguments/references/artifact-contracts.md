...
Исполняемое ядро проверки требует `case_id`, массивы `findings` и
`hypotheses`, а также `portfolio`. Оно проверяет типы всех перечисленных выше
полей `ResearchFinding` и `ArgumentHypothesis`, уникальность их собственных
идентификаторов, изоляцию дела и существование ссылок между findings и
гипотезами. В переносимом ядре `portfolio` обязательны
`human_approval`, допускающий `null` `principal_hypothesis_id` и массивы
`reserve_hypothesis_ids`, `experimental_hypothesis_ids`,
`rejected_hypothesis_ids`. При `human_approval=approved` дополнительно нужны
известная principal-гипотеза и непустая строка `approved_by`.

Пустые массивы исследования и дополнительные поля расширения допустимы.
Содержимое `hard_gates`, `dimension_comparison`, `critic_findings`,
`approval_reason` и расширения ЕСПЧ этот CLI пока не проверяет. Код `0` и
строка `OK:` означают только соответствие базовой структуры и ссылок этому
исполняемому ядру. Они не подтверждают достоверность источников, юридическую
достаточность, готовность жалобы или одобрение подачи.
