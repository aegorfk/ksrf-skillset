...
Гипотеза может включить `finding_id` в supporting/adverse список только если её собственный непустой строковый `hypothesis_id` явно присутствует в `finding.hypothesis_ids`; глобального существования finding недостаточно, а невалидный id или необъявленный перенос между гипотезами блокируется.
