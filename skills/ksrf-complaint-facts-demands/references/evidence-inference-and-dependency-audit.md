...
- Юрист подтверждает действующее право, применённость нормы и уместность для КС РФ; специалист по оценке подтверждает разделение выборок, проверки утечки и исключения признаков, а также воспроизводимую трассировку. До внедрения нужен отдельно документированный и одобренный план реализации, Langfuse-трассировка, DeepEval и проверка на отложенной выборке, а также явное человеческое одобрение.
