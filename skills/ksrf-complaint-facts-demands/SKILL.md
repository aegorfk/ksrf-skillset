...
description: Скилл готовит факты, конституционный вопрос и просительную часть жалобы в КС РФ из материалов дела и утверждённого портфеля гипотез. Он применяется, чтобы связать точную редакцию нормы с явным или доказанным имплицитным применением и вредом, развести зависимые доказательственные каналы и количественный риск, создать SentenceEvidenceMap и предложить основную и более узкую формулы требования без подгонки фактуры.
---
...
3. Строй факты хронологически, оставляя статус заявителя, попытки реализовать право, непосредственный вред, применение нормы, сохранённые доводы и исчерпание. Пометь каждый существенный факт: `установлено судом`, `следует из документа и не оспорено`, `утверждение стороны`, `оспаривается` или `не подтверждено`.
   Если заявителей несколько, используй [индивидуальную проверку в объединённом деле](references/joined-applicants-individual-proof.md): общая нормативная проблема не заменяет отдельное доказательство статуса, применения, срока и результата каждого участника.
4. По каждой судебной стадии фиксируй довод, speaker role, вывод суда, смысл нормы и точное цитатное окно. Отличай применение от упоминания. При отсутствии прямой ссылки собирай conjunctive pack по `../ksrf-complaint-cycle/references/implicit-application-gate.md`; полный pack даёт `implicitly_applied_proven` только после проверки заранее созданного host-attested approval полного record и всех chain fingerprints, а любое отсутствующее звено — `application_unclear`.
   Если причинная линия проходит через несколько судебных стадий, мотивировка менялась либо суд указал несколько оснований результата, используй [разбор оснований по инстанциям](references/cross-instance-causal-drafting.md). Покажи судьбу каждого существенного мотива, проверь самостоятельность другого основания применительно к точному последствию и составь ограниченный причинный абзац с сильнейшим возражением и локаторами.
5. До связного текста собери карту `норма -> судебный смысл -> правовое последствие -> заявленный конституционный вред` по `references/norm-application-defect-map.md`. Если approved hypothesis использует более точную причинную структуру, добавь её как отдельную карту.
   Если заявлен пробел, сначала открой `references/norm-system-and-gap-qualification.md` и разведи нормативный, информационный, семантический и оценочный разрывы. Недостающий акт или несогласие с установленным решением не выдавай за молчание законодателя.
   Для общих, повторяемых и индивидуальных фактических посылок отдельно заполни `references/constitutional-facts-evidence-ledger.md`; системная статистика не заменяет факт применения к заявителю, а единичный случай не доказывает общий механизм. Если один материал повторён через сторону, эксперта и суд и независимость каналов существенна, заполни там же `ConstitutionalFactProofRoute`: ссылайся на канонические `origin_id` и `dependency_group` общего Dependency DAG, а число экземпляров каналов и число независимых групп считай раздельно. Если акт оспаривается как фактически неверный или данных вообще не собирали, там же раздели `fact_assembly`, `fact_deployment` и возможное `reviewer_own_assembly`, затем проверь ограниченный `SystemicDataAcquisitionGap`; полезность недостающих данных не создаёт обязанности их собирать. Если вывод зависит от истории либо фактического нарратива прежнего решения, заполни `HistoricalClaimRoleAndFactualPrecedentAudit`: отдели holding от исторического факта, перепроверь первичный record, контекст, встречную версию и российский правовой якорь.
...
6. Для доказательственного спора укажи предмет, бремя, стандарт, доступ к сведениям, последствие недоказанности и нормативный носитель. Затем пройди `references/evidence-inference-and-dependency-audit.md`: отследи смену burden/presumption по стадиям, проверь rival explanation и evidential anchors, построй dependency graph против двойного учёта. Без носителя не превращай переоценку доказательства в конституционный вопрос; научные и Legal-AI модели остаются critic/eval и не устанавливают факт.
   Если существенный вывод выражен вероятностью, частотой, прогнозом, баллом, диапазоном или категорией риска, открой [проверку и изложение количественного риска](references/quantified-risk-communication.md). Раздели событие, горизонт, популяцию, версию инструмента, оценку, диапазон, calibration, cutoff и его владельца, ошибки двух типов, переход от группы к заявителю, реальную роль показателя и самостоятельное основание решения.
   Если неблагоприятный акт опирается на реестр, автоматическое сопоставление, скоринг, классификатор или иной цифровой output, дополнительно открой `references/automated-adverse-decision-record.md`: раздели ошибку данных, дефект процедуры и дефект нормативной конструкции; установи версию, входы, критерии, реальность human review, audit trace и причинную роль. Без нормативного носителя или связи output с исходом верни `ABSTAIN_NORMATIVE_BRIDGE`.
...
- `references/constitutional-facts-evidence-ledger.md` — уровни фактов, StageFactMap, разделение сборки/использования факта, ограниченный systemic-data gap, роли исторического тезиса и повторная проверка фактического нарратива прежнего акта без импорта иностранного процессуального режима.
