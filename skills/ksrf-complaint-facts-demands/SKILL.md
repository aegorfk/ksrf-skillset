...
9. Подготовь не менее двух уровней remedy, если это юридически возможно:
   ...
   Это исследовательские варианты, а не обязательное число пунктов просительной части. В текст включай выбранные требования; сохраняй их состав и ручные деления по [последней редакции пользователя](../ksrf-complaint-cycle/references/complaint-presentation-and-style.md#последняя-редакция-пользователя). Удалённую пользователем альтернативу не восстанавливай автоматически; правовое значение удаления и незакрытые вопросы отрази отдельно. В разделе III сначала изложи правовую подводку о полномочиях КС РФ, самостоятельно выбрав и проверив точные нормы. Затем поставь отдельный полужирный центрированный «ПРОШУ:» и требования с буквальными номерами 1., 2.: первое связывает выявленный нормативный дефект или неопределённость с проверкой соответствия Конституции; второе касается пересмотра дела при наличии законных оснований в случае принятия постановления. Пересмотр не становится безусловным и не подменяется просьбой к КС отменить судебные акты. После требований идут приложения. Для структурированной модели подводка передаётся как `request_basis`, требования — как `requested_remedy`; renderer не выбирает нормы и не дописывает юридический текст. Сохраняй пользовательский регистр маркера; дату и место подписи оформи одной строкой по [стандарту оформления](../ksrf-complaint-cycle/references/document-formatting-standard.md).
