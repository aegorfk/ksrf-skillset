...
Составляй саму жалобу по [макроструктуре и стилю судебного обращения](../ksrf-complaint-cycle/references/complaint-presentation-and-style.md), а причинные карты, статусы, риски и редакторские замечания сохраняй отдельно. Три основных раздела: фактические обстоятельства; позиция заявителя с конституционно-правовым обоснованием; требование к Суду. Формальные сведения входят в шапку и вводные абзацы. Представитель появляется только по прямому указанию пользователя. Нужные неизвестные сведения — конкретные жёлтые заглушки, а не служебные оговорки в абзацах. Существенные неблагоприятные выводы суда из фактов не исключай; предмет и просьбу выражай положительно, сохраняя необходимые отрицательные правовые суждения.
...
3. Строй факты хронологически, оставляя статус заявителя, попытки реализовать право, непосредственный вред, применение нормы, сохранённые доводы и исчерпание. Во внутренней карте фактов и отдельном Markdown пометь каждый существенный факт: `установлено судом`, `следует из документа и не оспорено`, `утверждение стороны`, `оспаривается` или `не подтверждено`. В самой жалобе передай авторство обычной фразой «суд установил», «заявитель ссылался», без служебных ярлыков.
...
8. Сформулируй исследовательский вопрос КС РФ для каждой жизнеспособной линии, сохраняя один фактический baseline. Покажи, чем principal и reserve отличаются по механизму, тесту или remedy. В разделе II жалобы изложи подтверждённый ответ как утвердительный довод, без вопросительных предложений; незакрытый вопрос сохраняй во внутреннем анализе.
9. Исследуй основную формулу и более узкий вариант remedy, если у них есть самостоятельное правовое обоснование; не создавай альтернативу ради количества:
   ...
   Это исследовательские варианты, а не обязательное число пунктов просительной части. Соблюдай [универсальность методики](../ksrf-complaint-cycle/references/complaint-presentation-and-style.md#универсальность-методики): не переноси персоналии, фактуру, нормы и готовые требования из образца. В текст включай выбранные требования; сохраняй их состав и ручные деления по [последней редакции пользователя](../ksrf-complaint-cycle/references/complaint-presentation-and-style.md#последняя-редакция-пользователя). Удалённую пользователем альтернативу не восстанавливай автоматически; правовое значение удаления и незакрытые вопросы отрази отдельно. В разделе III сначала изложи правовую подводку о полномочиях КС РФ, самостоятельно выбрав и проверив точные нормы. Затем поставь отдельный полужирный центрированный «ПРОШУ:» или «ПРОСИМ:» и последовательно пронумерованные требования. Первое связывает выявленный нормативный дефект или неопределённость с проверкой соответствия Конституции; число и содержание дополнительных требований определяются делом и допустимым результатом. Пересмотр включай отдельно, только если он применим и выбран, при наличии законных оснований в случае соответствующего решения КС РФ. Пересмотр не становится безусловным и не подменяется просьбой к КС отменить судебные акты. После требований идут приложения. Для структурированной модели подводка передаётся как `request_basis`, требования — как `requested_remedy`; renderer не выбирает нормы, число заявителей и требования, не дописывает юридический текст. Сохраняй пользовательский регистр и число маркера; дату и место подписи оформи одной строкой по [стандарту оформления](../ksrf-complaint-cycle/references/document-formatting-standard.md).
