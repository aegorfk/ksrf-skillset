...
- Каждый `drafting_blocks[].authority_ids[]` должен указывать на authority, где `hypothesis_ids` содержит `hypothesis_id` этого drafting block; authority может относиться к нескольким гипотезам, но каждая ссылка блока проверяется отдельно.
