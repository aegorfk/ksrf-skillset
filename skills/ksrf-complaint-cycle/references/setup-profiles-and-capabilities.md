...
Для верхнего состояния всего выбранного профиля `ready` и `degraded`
завершают doctor кодом `0`: работа возможна в явно показанных пределах.
`blocked` завершается кодом `3`, но полный отчёт с причиной и следующим шагом
остаётся в stdout. Неверные параметры или повреждённый манифест завершаются
кодом `2`. Код `3` не разрешает автоматически устанавливать программы,
создавать учётные записи или выполнять предложенное исправление.
