...
- [Не один score, а три оси](#не-один-score-а-три-оси)
- [Четыре совместимых итоговых статуса](#четыре-совместимых-итоговых-статуса)
- [Conjunctive test для implicitly_applied_proven](#conjunctive-test-для-implicitly_applied_proven)
- [Когда допустимо not_applied](#когда-допустимо-not_applied)
- [Цепочка инстанций](#цепочка-инстанций)
- [Admissibility gate](#admissibility-gate)
...
6. полный диагностический именной review-marker подтверждает промежуточную проверку этих звеньев.

Нет хотя бы одного звена → `application_unclear`.

Этот conjunctive test определяет промежуточный classifier status. Для
admissibility и release дополнительно нужен заранее созданный host-attested
approval fingerprint полного канонического record и fingerprints всех
использованных записей цепочки. Диагностический marker его не заменяет.

Этот итог относится к доказанности применения, причинно связанного с
оспариваемым последствием, и не стирает отдельные оси. Если record с
неопровергнутыми premise-level conclusions, полными court-authored locators и
полным диагностическим именным review-marker поддерживает вывод о
воспроизведении нормативной логики, но тот же
исход удерживает самостоятельное достаточное основание, сохрани
...
Если premise имеет `inference_status=contradicted`, diagnostic review
`pending`, `rejected` или `needs_changes`, либо отсутствует именной reviewer и
review timestamp, не используй причину `implicit_norm_use_preserved`. Сохрани
`application_unclear` с `implicit_norm_use_not_verified` и отдельным causal
blocker. Raw `human_review` намеренно исключён из fingerprinted review payload;
поэтому именной marker — лишь диагностический признак, а не одобрение точного
complete record. Он не заменяет host-attested trusted approval для
admissibility/release и не даёт полномочия на подачу.
...
Пригодный полный `independent_ground` span, атрибутированный суду или
резолютивной части, должен согласовываться с
`outcome_causation=independent_sufficient_ground`. Несогласованный record
отклоняй как внутренне противоречивый до классификации: нельзя оставить
основание только среди evidence и одновременно получить положительный вывод из
обычной determinative-ветки. Дубликаты premise names также не нормализуются по
порядку, а отклоняются схемой и runtime.
...
Поздний суд наследует мотивировку только при доказанном incorporation locator и
непустой court-authored цитате. Locator без quoted content, противоречащий span
или простое оставление акта без изменения не доказывают принятие каждого мотива.
