...
Применяй [единый макет первой страницы](docx-first-page-layout.md) и [структуру судебного обращения](complaint-presentation-and-style.md). Это конвенция оформления, а не новое требование допустимости. Шапка начинается от половины полезной ширины строки, выравнивается внутри по левому краю, имеет именованный стиль `KSRF Complaint Header`, Times New Roman 11,5 pt и одинарный интервал. Представитель включается только по прямому указанию пользователя. После шапки — две центрированные строки: «ЖАЛОБА» / «на нарушение конституционных прав и свобод».

Во всем DOCX/PDF, включая колонтитулы и свойства документа, отсутствуют автоматически добавленные workflow-статусы, риск самоанализа, sentence/Matter IDs и служебные инструкции. Пробелы и QA хранятся в отдельном Markdown и служебных данных. Конкретные незаполненные реквизиты выделяются жёлтым highlight; строгий release по-прежнему блокируется незаполненными данными и неподтвержденными опорами. Существенные неблагоприятные мотивы суда сохраняются в фактах или обосновании.

Визуально проверь первую страницу и весь DOCX/PDF: размещение шапки, переносы реквизитов, читаемость, заголовки, выделение заглушек, отсутствие обрезаний, наложений и пустой последней страницы. Укажи реально просмотренный объем в отдельном отчете.
