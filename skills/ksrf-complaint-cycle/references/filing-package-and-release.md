...
- [Structured complaint](#structured-complaint)
- [Артефакты](#артефакты)
- [Проверка DOCX/PDF](#проверка-docxpdf)
- [Шапка жалобы](#шапка-жалобы)
- [Статусы](#статусы)
- [Инвалидация](#инвалидация)
- [Человеческая граница](#человеческая-граница)

## Structured complaint

Рабочая structured-complaint модель `1.4` содержит обязательные разделы жалобы, `matter_id`, `draft_id`, source/version bindings, norm passport IDs, plural selected issue option IDs, approvals, formal check и `SentenceEvidenceMap`. Filing-package manifest `1.5` всегда несёт отдельные relief-, application-finding-, legal-holding- и practice projections, массивы receipts, специализированные host-authoritative index receipts и полный `sentence_role_index_receipt`. В диагностическом `blocked` manifest допустимы blockers, `unbound`, пустые receipt-массивы и `null` index. Неизвестную явно указанную роль можно сохранить в таком manifest, включая раздел просительной части, только вместе с отдельным точным blocker `sentence_role_unknown:<sentence_id>:<исходная_роль>`; префикс, суффикс, иной ID либо иная роль не соответствуют строке. Это сохраняет строку для исправления, но не создаёт release authority. Любой ready-статус требует `blockers=[]`, только канонические роли, ненулевой current `sentence_role_index_receipt`, `bound`-строки, current relief receipts `1.1.0`, current application-finding receipts для каждой строки `application_finding`, current holding receipts `1.0.0` для каждой строки `legal_holding`, current practice receipts `1.0.0` для каждой строки `practice_claim` и все специализированные current index. Application-finding-, holding- и practice-index обязательны даже при authoritative `bindings=[]`; пустой receipt-массив допустим только при отсутствии строк соответствующей роли. Manifest `1.0`/`1.1`/`1.2`/`1.3`/`1.4` остаётся только историческим JSON и должен быть пересобран перед новым release approval. Singular `issue_option_id` остаётся только диагностическим alias и не создаёт membership для нескольких линий.

`application_finding` — отдельный release gate, а не подпись к произвольному evidence ID. Строка хранит exact текст, `claim_id`, `norm_passport_id`, strict unique `application_record_ids`, strict unique `evidence_ids` и `maximum_supported_inference`; неполная legacy-строка остаётся видимой как `unbound`, но ready-статус не получает. Host resolver независимо от caller selection возвращает полный ordered chain inventory с current record fingerprints и собственными `chain_revision_id`/`checked_at`: выбранные записи должны иметь положительную application classification, `outcome_causation=determinative|contributory`, current norm/preservation/application approvals и входить в surviving support. Финальный акт, который только прямо включает положительную нижестоящую запись, остаётся частью полной цепочки, но не получает фиктивный positive-record receipt; каждый named incorporated record обязан однозначно разрешаться в более раннюю запись полного inventory. Exact evidence set включает только non-contradicted full-act court spans direct proof (`express_norm_use`, `operative_rule`, `outcome_link`) либо доказательства, соответствующие роли/speaker каждой конкретной implicit premise, включая `outcome_link|counterfactual_analysis` для причинности и human-confirmed reviewer `alternative_ground_analysis`, а также валидный locator включения, если chain status — `incorporated`; contradicted/foreign/party/background/quotation-only span не проходит. Scope receipt отдельно связывает byte-equal wording, предел вывода, тот же passport, selected sets и полный chain fingerprint/revision/check time. Каждый ready manifest несёт current receipt каждой строки и host-owned полный application-finding index, включая authoritative empty set; top-level authority revision per-line receipts и index совпадает, но не подменяет отдельную ревизию evidence chain. Любая вставка, удаление, role downgrade, omitted later act, unresolved incorporation, stale approval или wording drift блокирует выпуск и требует нового exact review.
...
Это конвенция оформления документа, а не самостоятельное требование КС РФ или порог допустимости. Применяй её при подготовке жалобы, если пользователь явно не задал иной макет.

- Блок адресата, заявителя и представителя размещай в правой половине листа: его левая граница проходит примерно по геометрической середине физической страницы. Для A4 шириной 21 см это 10,5 см от левого края листа; при левом поле 2,5 см задай `left_indent = 8 см` относительно поля. При правом поле 2,5 см доступная ширина блока также равна 8 см.
- При других полях рассчитывай `left_indent = page_width / 2 - left_margin`; ширина блока до правого поля равна `page_width / 2 - right_margin`. Используй одинаковые единицы измерения. Не подменяй середину листа серединой текстового поля.
- Задавай положение абзацным отступом или стилем, не пробелами. Все строки внутри блока, включая перенесённые, выравнивай по левому краю; отступ первой строки — нулевой. Правое выравнивание строк не заменяет размещения блока справа.
- Сохраняй обычный кегль документа. Длинные реквизиты переноси на следующие строки; не уменьшай шрифт ради уместимости шапки.

В визуальном QA финальной первой страницы проверь общую левую границу строк около середины листа, переносы длинных реквизитов, сохранение кегля, отсутствие обрезаний и наложений. После конвертации в PDF проверь геометрию снова. Настройки абзаца и успешный экспорт не доказывают визуальную исправность: укажи фактически просмотренные файлы/страницы либо непроверенный объём.
