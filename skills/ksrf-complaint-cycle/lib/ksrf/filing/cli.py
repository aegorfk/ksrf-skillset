...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor_exit_help = (
        "Коды завершения диагностики: 0 — основные возможности готовы (ready) "
        "либо работа возможна с показанными ограничениями (degraded); 2 — "
        "некорректные параметры либо файл описания возможностей не найден или "
        "повреждён; 3 — есть блокирующие пробелы (blocked) либо итоговое "
        "состояние отчёта не распознано. При коде 3 полный отчёт с причинами и "
        "следующим действием остаётся в стандартном выводе (stdout). "
        "Диагностика ничего не устанавливает и не исправляет автоматически. "
        "Ни один код не подтверждает юридическую готовность жалобы и не "
        "разрешает её подачу или передачу документов."
    )
    doctor_parser = commands.add_parser(
        "doctor",
        help=doctor_description,
        description=doctor_description,
        epilog=doctor_exit_help,
    )
