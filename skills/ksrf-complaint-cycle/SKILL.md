...
4. Зафиксируй, кто действует, имеется ли продолжающееся дело, какая норма применена, кто ставит вопрос о конституционности и какой результат требуется.
5. Если даны материалы, проект или папка, запусти автосбор по `references/ksrf-tool-layer.md` и используй `CaseFile`. Не выдавай автоматически извлечённый кандидат за подтверждённую норму или факт.
...
7. Не приравнивай отсутствие full-text hit к отсутствию документа. Если OCR недоступен, верни технический gap, а не содержательный вывод о составе дела.
...
2. До формальной подачи финализируй `FilingDecisionRecord`, если сработал его триггер: требуется информированное решение клиента и отдельное одобрение юриста без давления символической или общественной целью.
