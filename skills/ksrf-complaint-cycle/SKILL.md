...
description: "Скилл помогает заявителю без юридической подготовки пройти полный цикл обращения в Конституционный Суд РФ: начать с обычного рассказа, номера дела, документов, проекта, письма Секретариата или акта КС РФ. Он диагностирует доступную среду, сам разбирает материалы, проверяет источники, применение нормы и процессуальный путь, исследует варианты, готовит рабочие DOCX/PDF и отдельно проверяет комплект перед человеческим одобрением, подписанием и подачей."
---
...
Для нового пользователя и установки плагина `ksrf-applicant` сначала прочитай [понятный вход и полный каталог маршрутов](references/applicant-start.md). Прими обычный рассказ, номер дела, доступные документы, готовый проект или полученный ответ. Сам выбери начальный маршрут по содержанию; не требуй юридической анкеты, выбора навыка, JSON или работы в терминале. Если материалы уже переданы, сразу начни их разбор. Используй все профильные навыки по задаче, включая исследование, подготовку текста, проверку и действия после акта КС РФ.

Для первого проверяемого документа и разграничения гипотезы, рабочего проекта и строгого выпуска прочитай [практический локальный путь](references/practical-working-route.md). При незакрытых доказательствах доступен `render draft`: DOCX/PDF в форме судебного обращения, жёлтые заглушки неизвестных данных и отдельный Markdown-отчет о пробелах. Его создание не требует удостоверения выпуска и не повышает готовность к подаче.

1. Прочитай [машину состояний](references/router-and-state-machine.md).
2. Если пользователь установил только skills либо setup неизвестен, прочитай [профили](references/setup-profiles-and-capabilities.md) и покажи все три профиля понятными названиями: базовый (`basic`), расширенное исследование (`research`), экспертная проверка (`expert`). Если профиль не выбран, явно назови базовый рекомендуемым начальным режимом. В среде с исполнением выполни `scripts/ksrf.py start --profile basic --json`, затем `scripts/ksrf_setup_doctor.py --profile basic --json`; в плагине доступны те же команды через переносимый адаптер, описанный в [первом запуске](references/applicant-start.md#среда-и-первый-запуск). В среде без исполнения честно отметь, что эти проверки не выполнены, и продолжи доступную работу с текстом и файлами. Отсутствие инструмента не является отрицательным юридическим выводом. Ничего не устанавливай и не передавай вовне автоматически.
3. При доступе к локальным файлам создай или открой matter по [контракту workspace](references/matter-workspace-and-artifacts.md), вне установленного пакета. Если среда не даёт постоянного хранилища, веди явный доступный пользователю перечень материалов и состояния и не утверждай, что создан локальный workspace. Пользовательские материалы по умолчанию `local_confidential`; при remote operation сначала примени [privacy gate](references/privacy-and-case-isolation.md). Загрузка документа в ChatGPT уже передаёт его сервису ChatGPT и не является локальной обработкой.

## Написание и предлагаемая редакция

Для любой жалобы применяй [структуру и стиль обращения к Суду](references/complaint-presentation-and-style.md): шапка, двухстрочный заголовок, факты, позиция с конституционным обоснованием, требование, приложения и подпись. DOCX/PDF содержит судебный текст; статусы, риски и самопроверка — отдельный Markdown. Не добавляй представителя без прямого указания пользователя. Неизвестные данные выделяй жёлтым highlight; их удаление из видимой маркировки не меняет Unknown, доказательства или `filing_ready=false`.

При поручении написать или улучшить жалобу используй [сохраняемый редакторский цикл](references/complaint-writing-loop.md): концепция → доводы с точными основаниями → возражения → предлагаемая правка → повторная проверка. Сам извлекай доступные данные и готовь входы CLI `writing`; пользователю показывай документы, изменения и пробелы. Разрешена автоматическая подготовка отдельной редакции в пределах поручения, с сохранением исходника и человеческим принятием текста.

Если предполагаемый дефект связан с качеством законодательной, административной или судебной процедуры, маршрутизируй его через [process-based review workbook](../ksrf-rights-argument-builder/references/process-based-rights-review-workbook.md) и [процедурный QA](../ksrf-complaint-qa/references/procedural-adequacy-and-cure.md). Процедура должна быть причинно привязана к применённой норме и материальному либо самостоятельному процессуальному вреду; формальный недостаток не равен неконституционности.
...
Если применённая норма изменена, отменена, заменена, перенумерована либо изменился её судебный смысл, используй [сопоставление нормативного смысла](references/norm-meaning-continuity.md). Тот же справочник применяй, когда после позиции КС РФ вред связывают с воспроизводящей, производной, отсылочной или функционально аналогичной нормой. Сравни прежнее и последующее регулирование по шести элементам, отдельно докажи применение поздней нормы и не выводи её судьбу из одного сходства. Раздельно установи прошлое применение, доказанное дальнейшее применение к старым отношениям, сохраняющийся вред и влияние новой редакции. Передай вывод в обычную проверку допустимости; эта методика не создаёт нового gate, права на повторную жалобу или нового срока.
...
Результат: `GO_TO_KSRF`, `FIX_FIRST`, `COURT_REQUEST_ROUTE`, `NO_GO_KSRF` либо `ABSTAIN_PENDING_RECORD`. Даже `GO_TO_KSRF` остаётся рекомендацией для юриста: `human_decision=pending`, `legal_assessment_automated=false`, `filing_authority=false` и `filing_performed=false`. При critical unknown сохраняй `filing_ready=false` в служебных данных; оформление судебного текста не является разрешением подписать или подать жалобу.

### 5. Исследуй варианты и отказные аналоги

Когда для выявления проблемы или поддержки довода полезны зарубежные акты, подключай [сравнительное исследование](../constitutional-comparative-research/SKILL.md) по его [агентному маршруту](../constitutional-comparative-research/references/agent-research-workflow.md). Агент использует доступный MCP, читает полные источники и закрывает пробелы официальным поиском; КС РФ, ВС РФ и HUDOC сохраняют собственные маршруты и юридические роли. Зарубежная база и её векторный индекс не являются обязательной предпосылкой подготовки российского рабочего проекта.
...
Соблюдай [универсальность методики](references/complaint-presentation-and-style.md#универсальность-методики): правила не зависят от персоналий, частного образца или заданного результата дела. Предмет, факты, нормы и требования устанавливаются для текущего обращения.

Соблюдай [структуру и прямой стиль](references/complaint-presentation-and-style.md): в разделе II собственные доводы изложены утвердительно, исследовательские вопросы остаются в анализе; в III авторская правовая подводка предшествует отдельному центрированному «ПРОШУ:» или «ПРОСИМ:», выбранным последовательно пронумерованным требованиям и приложениям. Число и содержание требований определяются делом; пересмотр не добавляется автоматически. Правовые основания проверяет автор; renderer сохраняет смысл. Абзацы с источниками оформляются как основной текст по [выбранному стандарту](references/document-formatting-standard.md).
...
Для социальных гарантий, юридических приравниваний и санкций используй [юридический эффект и время гарантии](../ksrf-rights-argument-builder/references/legal-effect-and-guarantee-timing.md). Подборки ведомственных и экспертных ответов проверяй через [карту конкурирующих заключений](../ksrf-doctrine-research/references/adversarial-opinions-and-tax-harm.md). Сначала собственный довод, затем сравнение с историческим исходом; иск в обычный суд не называй жалобой в КС РФ.
...
Для экологического взыскания после восстановительных работ открой [методику состава возмещения](../ksrf-rights-argument-builder/references/environmental-compensation-and-restoration.md): отдели результат работ от затрат, конституционный дефект от спора о доказательствах, а итог нормоконтроля от условий пересмотра.
