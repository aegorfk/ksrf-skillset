...
Retrieval работает от bundled references и официальных актов; публичные каналы для него не обязательны. Для обнаружения новых актов и тематических подборок можно дополнительно использовать публичные каналы и юридические медиа как discovery layer, но не как финальную правовую опору. Исторический корпус `@ksrf_ru` использован для выработки такого порядка:
