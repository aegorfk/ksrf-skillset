...
Все CLI-команды, принимающие `request.json`, используют тот же строгий parse gate: повреждённый, не UTF-8-кодируемый или содержащий не конечные числа вход блокируется контролируемой ошибкой `invalid JSON artifact: request.json` (код 2), без traceback. Для команд с `--workspace` такая ошибка также перезаписывает устаревший `qa-report.json` в `status=fail`. Любая контролируемая ошибка до начала записи производных поисковых артефактов в `plan` или `search` (включая bound-plan, provider-access и adapter preflight) обязана тем же образом инвалидировать прежний `qa-report.json`; успешный запуск не должен создавать такой failure-report.
