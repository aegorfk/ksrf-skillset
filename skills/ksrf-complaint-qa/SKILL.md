...
12. Не ставь `готово`, пока для principal и reserve не указан юридически допустимый результат и проверяемый маршрут его реализации после решения. Для диалогового результата требуй участников, предмет, временную защиту, срок, отчёт, контроль, эскалацию и российское полномочие.
13. Сравни активные гипотезы по измерениям из `../ksrf-explore-arguments/references/evaluation-and-promotion.md`. Не сворачивай проверку в разрешающий scalar score.
14. Выдай verdict и section-level rewrite plan. `готово` означает только переход к `ksrf-formal-filing-check` после явного решения человека.
