...
`quality coding-audit-prepare` читает готовые артефакты корпуса, проверяет content identity всех захваченных текстов до фильтрации по запросам и одним переименованием публикует отдельный неизменяемый каталог первичных входов аудита. Обязательный `--codebook-version 1.0` выбирает штатный справочник независимо от ответов первого кодировщика и должен точно совпасть с версией во всех первичных карточках. Команда не изменяет workspace и не принимает существующий каталог назначения. В его трёх штатных identity-файлах `document_id` имеет точную форму `document-sha256:<64 lowercase hex>`.
...
| `independent-review-packet.zip` | Единственный передаваемый второму кодировщику файл; ровно пять payload-файлов (`CODING-BRIEF.json`, «CODING-CODEBOOK.md», «REVIEW-INSTRUCTIONS.md», `review-materials.jsonl`, `secondary-coding-template.jsonl`) и `review-packet-manifest.json` без первичной разметки | независимая проверка и сверка SHA-256 |
| `coding-audit-inputs-manifest.json` | SHA исходных данных и каждого файла пакета, полная рамка и обязательная выборка; одобрение и юридическая готовность всегда `false` | целостность |

Весь каталог является пакетом хранителя и содержит первичные ответы: передавать его независимому кодировщику нельзя. Передаётся только `independent-review-packet.zip`. `quality coding-audit-prepare` выводит точный SHA-256 этого архива в поле stdout `independent_review_packet_sha256`; хранитель сообщает проверяющему именно это значение по независимому каналу, отдельно от ZIP, а проверяющий сверяет его с SHA-256 полученного файла до распаковки.

`CODING-BRIEF.json` переносит из замороженного плана только нейтральный контекст, нужный для разметки: ровно одну направленную гипотезу `hypothesis_under_test`, нормы, совокупность и правила включения, исключения, исходозначимости и противоречия. Значения `supports` и `adverse` относятся к этой гипотезе; открытый вопрос или несколько гипотез требуют отдельных заново замороженных планов. Brief не содержит поисковых запросов, совпадений или ответов первого кодировщика. Файл «CODING-CODEBOOK.md» фиксирует штатную процедуру выбранной версии; внутренний manifest связывает точные bytes и SHA-256 обоих файлов.

Ослепление структурное и ограничено ответом первого кодировщика и дорожкой отбора: внутри ZIP нет первичной разметки и её хешей, имени первого кодировщика, screening matches/queries, `sample_lane` или adjudication. Полный текст судебного акта включён, поэтому факты, мотивировка и исход видны; это не `outcome-blind` оценка. Внутренний manifest фиксирует `publication_safe=false`, `review_state="independent_secondary_required"`, `human_approval_created=false` и `legal_readiness=false`. Архив требует адресной защищённой передачи и отдельного решения о публикации или редактировании полного текста.

`plan_sha256` и производный `candidate_id` остаются непрозрачной криптографической привязкой к полному frozen plan, включая query lanes. Сырые запросы и matches не сериализуются, но digest не является обещанием семантической тайны для того, кто знает план или способен перебрать его варианты.

Ни каталог, ни ZIP не являются доказательством согласия кодировщиков. Завершённые `audit-decisions.jsonl` и, при фактических расхождениях, `adjudications.jsonl` создаются отдельно людьми и только затем передаются в `quality coding-reliability`. До этого независимая проверка, human/legal review и filing gates остаются незавершёнными. Эта команда принимает только строгий UTF-8 JSON/JSONL без повторных ключей на любом уровне и без `NaN`/бесконечностей. Escaped lone surrogate не становится evidence: запись остаётся невалидной и получает только безопасный служебный отпечаток для диагностики.
