...
`quality coding-audit-prepare` читает готовые артефакты корпуса, проверяет content identity всех захваченных текстов до фильтрации по запросам и одним переименованием публикует отдельный неизменяемый каталог первичных входов аудита. Он не изменяет workspace и не принимает существующий каталог назначения. В его трёх штатных identity-файлах `document_id` имеет точную форму `document-sha256:<64 lowercase hex>`.
...
Пакет не является доказательством согласия кодировщиков. Завершённые `audit-decisions.jsonl` и, при фактических расхождениях, `adjudications.jsonl` создаются отдельно людьми и только затем передаются в `quality coding-reliability`. Эта команда принимает только строгий UTF-8 JSON/JSONL без повторных ключей на любом уровне и без `NaN`/бесконечностей. Escaped lone surrogate не становится evidence: запись остаётся невалидной и получает только безопасный служебный отпечаток для диагностики.
