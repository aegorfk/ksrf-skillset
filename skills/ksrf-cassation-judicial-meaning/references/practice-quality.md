...
- `independent-review-packet.zip` — единственный файл, предназначенный для передачи второму кодировщику; внутри ровно пять payload-файлов и один manifest: `CODING-BRIEF.json`, «CODING-CODEBOOK.md», «REVIEW-INSTRUCTIONS.md», `review-materials.jsonl`, `secondary-coding-template.jsonl` и `review-packet-manifest.json`;
...
Рамка и первичные решения охватывают всех кандидатов; очередь, пустые шаблоны и материалы внутри ZIP содержат только `required_candidate_ids` из замороженного audit-плана. Так независимый проверяющий получает ровно выбранную работу, а полнота исходного знаменателя остаётся проверяемой.

### 2. Передай независимому кодировщику только ZIP

Не передавай весь каталог `$AUDIT_BUNDLE`: он является пакетом хранителя и содержит ответы первого кодировщика. Передай выбранному проверяющему только `$AUDIT_BUNDLE/independent-review-packet.zip`. Команда подготовки показывает точный SHA-256 этого ZIP в поле машинно-читаемого stdout `independent_review_packet_sha256`. Сообщи проверяющему именно это значение по независимому каналу, отдельно от самого архива. После получения проверяющий должен вычислить SHA-256 файла и сравнить его с этим значением до распаковки, например:

```bash
shasum -a 256 independent-review-packet.zip
```

В `CODING-BRIEF.json` находится нейтральная, связанная с замороженным планом постановка работы: ровно одна направленная гипотеза со статусом `hypothesis_under_test`, нормы, совокупность и правила включения, исключения, исходозначимости и противоречия. `supports` и `adverse` относятся именно к этой гипотезе. Открытый `research_question` или несколько гипотез команда не упаковывает: сначала переформулируй и заново заморозь отдельный план для каждой гипотезы. В brief нет поисковых запросов, совпадений или ответов первого кодировщика. Файл «CODING-CODEBOOK.md» — штатный справочник выбранной версии с правилами для всех обязательных полей; его точные bytes и SHA-256 связаны внутренним manifest.

Такое ослепление является структурным и ограниченным: в ZIP нет первичной разметки, её хешей, имени первого кодировщика, поисковых совпадений и указания, из какой дорожки отбора (`sample lane`) кандидат попал в объединённую выборку. Это не `outcome-blind` оценка и не сокрытие материалов дела. В `review-materials.jsonl` находится полный текст судебного акта, поэтому проверяющий видит факты, мотивировку и исход и может встретить персональные или иные чувствительные сведения.
...
Внутренний manifest фиксирует `publication_safe=false`, `review_state="independent_secondary_required"`, `human_approval_created=false` и `legal_readiness=false`. Поэтому ZIP предназначен для адресной защищённой передачи, а не для публикации. Его создание и проверка SHA не означают, что независимая разметка выполнена, расхождения разрешены, результат одобрен юристом или жалоба готова к подаче.

Каталог и ZIP — воспроизводимые входы, а не результат независимой проверки. Неизменённый `secondary-coding-template.jsonl` намеренно не проходит `coding-reliability`. Второй человек должен независимо прочитать полный текст из ZIP, заполнить отдельную копию шаблона, указать другого `coder` и проверить цитату. Затем хранитель формирует отдельный четырёхполевой `audit-decisions.jsonl`, не раскрывая первичную разметку до завершения вторичной. При реальном расхождении отдельный человек готовит `adjudications.jsonl`. Команда подготовки не выполняет вторую разметку, разрешение расхождений, юридическое одобрение или подачу жалобы.

### 3. Экспертный ручной путь остаётся совместимым
