...
Сохраняй строку приватно: видимый `message_ru` может содержать чувствительные имена записей (`basename`) и координаты устройства/inode (`st_dev`/`st_ino`). `administrator_only` для `staging_cleanup_uncertain` и `publication_state_uncertain` означает остановку и передачу состояния системному администратору для полного учёта имён/жёстких ссылок и карантина, без пользовательского повтора, удаления, передачи или использования. `repeat_then_compare_candidate` для `publication_durability_uncertain`, `publication_finalization_uncertain` и `confirmation_delivery_uncertain` — только кандидат, не разрешение: сохрани всё неизменным, устрани внешнюю причину, используй новую отсутствующую соседнюю папку, получи обычный успешный повтор с кодом `0` и лишь затем побайтово сравни каталоги. Для подготовки, импорта и финализации используй установленные `quality native-reliability compare-audit-bundles`, `quality native-reliability compare-review-imports` и `quality native-reliability compare-finalizations` соответственно.

Для `compare-audit-bundles` передай две разные полные приватные соседние папки в `--uncertain-audit-bundle-dir` и `--repeated-audit-bundle-dir`, а также оба SHA из одной полной строки stdout нормально завершившегося повтора в `--expected-manifest-sha256` и `--expected-independent-review-packet-sha256`. Не восстанавливай ожидаемые SHA из пакетов. Команда только читает: проверяет оба пакета и установленный справочник, связывает оба внешних SHA с повторным пакетом, побайтово сравнивает все семь файлов и заново проверяет снимки. Коды: `0=match`, `3=mismatch`, `2=invalid/unreadable`. Даже `match` не доказывает исходную допустимость восстановления, нормальный возврат повтора, происхождение SHA, долговечность первого пакета или исходную рабочую папку и не разрешает дальнейшее использование. Только повторный пакет идёт на новые проверки получателя; проверяющему предназначен лишь ZIP с отдельным сохранённым SHA и обычной проверкой допустимости защищённой передачи. Копируемая команда и условия приведены в `references/practice-quality.md`.
