...
def candidates() -> list[Path]:
    values: list[Path] = []
    configured = os.environ.get("HUDOC_KB_CLI")
    if configured:
        _append_unique(values, Path(configured))
    for repository in repository_candidates():
        _append_unique(values, repository / "scripts" / "hudoc_knowledge_base.py")
        for worktree in repository_worktrees(repository):
            _append_unique(values, worktree / "scripts" / "hudoc_knowledge_base.py")
    return values
