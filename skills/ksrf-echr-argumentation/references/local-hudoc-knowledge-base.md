...
- стабильный resolver CLI: `scripts/hudoc_kb_cli.py` внутри текущего skill; если задан `HUDOC_KB_CLI`, он проверяет только этот точный путь, иначе требует `HUDOC_KS_PARSER_REPO` и ищет совместимый CLI только в явно указанном корне и его Git worktrees; запускается лишь связка `hudoc-knowledge-indexer-v3.8` + `hudoc-research-extractive-v7` + `hudoc-knowledge-privacy-sanitizer-v2`;
- hybrid resolver CLI: `scripts/hudoc_vector_cli.py` внутри текущего skill; если задан `HUDOC_VECTOR_CLI`, он проверяет только этот точный путь, иначе использует тот же явно настроенный `HUDOC_KS_PARSER_REPO` и его Git worktrees; проверяет `hudoc-vector-indexer-v2` + `hudoc-vector-evaluator-v2` + KB v3.8 + research v7 + privacy v2 и использует отдельный HUDOC Qdrant на `127.0.0.1:6335`. Чужие Qdrant collections не выбирать.

Сам движок HUDOC и его индекс не входят в пользовательскую установку skills. Resolver не ищет их по `HOME` или текущему Git-репозиторию: нужно явно задать точный `HUDOC_KB_CLI` / `HUDOC_VECTOR_CLI` либо корень `HUDOC_KS_PARSER_REPO`. Если настройка отсутствует или несовместима, это пробел runtime-доступа, а не доказательство отсутствия релевантной практики ЕСПЧ.

До настройки точный одиночный `-h` или `--help` выводит локальную справку по первоначальной настройке только когда обе переменные настройки отсутствуют для соответствующего resolver; пустое, неверное или несовместимое значение остаётся ошибкой, а после настройки передаётся совместимому движку точная исходная последовательность аргументов, включая `--help`. Код 0 bootstrap-справки не подтверждает доступность движка, покрытие или актуальность корпуса, юридическую силу результатов либо готовность материалов для жалобы.
